...
/**
 * Entrada Xposed com inicialização tardia.
 *
 * O ReLSPosed pode carregar a classe antes de o Looper principal existir. Por isso
 * nenhum Handler é criado em inicializador estático; ele só nasce em Application.attach.
 */
public final class InlineActivationHook implements IXposedHookLoadPackage {
    ...
    private static final String HOOK_VERSION = "3.3.5-inline-lazy";
    private static final String INLINE_TAG = "threads_enhancer_inline_download_335";
    ...
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam param) {
        ...
        XposedBridge.log(
                "ThreadsEnhancer/ReLSPosed lazy bootstrap: pacote carregado em " + param.processName
        );
    ...
    private static void hookApplicationAttach() {
        try {
            ...
            XposedBridge.log("ThreadsEnhancer/ReLSPosed lazy attach: " + error);
    ...
    private static final class AttachCallback extends XC_MethodHook {
        ...
        protected void afterHookedMethod(MethodHookParam param) {
            ...
            // Application.attach já ocorre com o Looper principal preparado.
            if (mainHandler == null) {
                mainHandler = new Handler(application.getMainLooper());
            }

            XposedBridge.log("ThreadsEnhancer/ReLSPosed lazy bootstrap: Application.attach confirmado");
    ...
    private static Rect findShareActionBounds(Activity activity) {
        ...
        try {
            ...
            // Fallback para a árvore Android real abaixo.
        } finally {
            recycleQuietly(root);
        }
        ...
        try {
            ...
            for (int index = 0; index < childCount; index++) {
                ...
                try {
                    ...
                    // Um nó virtual pode mudar durante a recomposição.
                } finally {
    ...
    private static final class DownloadIconDrawable extends Drawable {
        ...
        public void draw(Canvas canvas) {
            ...
            canvas.drawLine(centerX, arrowBottom,
                    bounds.left + width * 0.31f, bounds.top + height * 0.48f, paint);
            canvas.drawLine(centerX, arrowBottom,
                    bounds.right - width * 0.31f, bounds.top + height * 0.48f, paint);
    ...
    private static void sendHeartbeat(Context context) {
        ...
        try {
            ...
            XposedBridge.log("ThreadsEnhancer/ReLSPosed lazy IPC: " + error);
        }
        ...
        if (!providerAccepted) {
            try {
                ...
                XposedBridge.log("ThreadsEnhancer/ReLSPosed lazy broadcast: " + error);
            }
        }

        XposedBridge.log(
                "ThreadsEnhancer/ReLSPosed lazy bootstrap: heartbeat enviado provider="
                        + providerAccepted
        );
    }
    ...
    private static void startFullHook(XC_LoadPackage.LoadPackageParam param) {
        try {
            Class<?> hookClass = Class.forName(
                    ...
                    true,
                    InlineActivationHook.class.getClassLoader()
            );
            ...
            XposedBridge.log("ThreadsEnhancer/ReLSPosed lazy delegate: " + error);
            XposedBridge.log(error);
        }
    }
}
